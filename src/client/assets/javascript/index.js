  ...
  document.addEventListener(
    ...
    function (event) {
      ...
      if (target.matches("#gas-peddle")) {
        handleAccelerate(store.track_id);
      }
    },
    false
  );
...
async function runRace(raceID) {
  try {
    ...
    return console.log("Problem with runRace function::", error);
  }
}
...
async function accelerate(id) {
  ...
  try {
    await fetch(`${SERVER}/api/races/${id - 1}/accelerate`, {
      method: "POST",
      ...defaultFetchOpts(),
    });
    console.log("accelerate returned");
  } catch (err) {
